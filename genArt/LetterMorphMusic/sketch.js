...
function setup() {
  //sets colour of objects according to GUI
  c = color(textColor);
  c2 = color(ringColor);
  c3 = color(circleColor);

  //unused peak detect method
  //fft = new p5.FFT();
  //peakDetect = new p5.PeakDetect(-10, 23000, 0.01,0.01); // set decay .99 for faster response

  peaks = currSound.getPeaks(); // for debugging

  //calculates average of all peaks accross current sound file
  averagePeak = peaks.reduce((p, c) => p + c, 0) / peaks.length;

  for (let i = 0; i < averagePeak.length; i++) {
    //remap peaks (-1 to 1) to levels (0 to 1)
  ...
  background(255); //to debug if draw has not executed

  //loads fonts
  opentype.load("data/VectFont.ttf", function (err, f) {
    // issues with A B D AND P
    if (err) {
      console.log(err);
    } else {
      font = f;
  ...
  opentype.load("data/BigBoy.otf", function (err, f) {
    // issues with positioning .otf files
    if (err) {
      console.log(err);
    } else {
      font2 = f;
  ...
  spline_size = width / 1.65; // initial splines density

  resetSpline(spline_size);

  currFont = font; //assign selected font

  //initialize gui selections
  fontSelector();
  musicSelector();
}

function draw() {
  //resize splines if gui updated
  ...
  //unused peak detection method using fft object
  //fft.analyze();
  //peakDetect.update(fft);

  //amplitude of current frame
  level = amplitude.getLevel();

  fill(255);
  background(0, 0, 0, a);

  //draw backgound elements
  drawEllipse();
  drawSplines();

  //user has added first word
  if (started) {
    //handles drawMessage for full shapes
    flag = false;
    mod = 0;
    ...
    if (first) {
      ...
      first = false;
      currSound.play();
      start(); // start calls shapearray to start transition to target array
    }

    push();
    //handles centering of text calculation
    translate(
      width / 2 - (width / fSize - spacing) * (shapeArray.length / 2),
      height / 2 + width / fSize / 2
    );
    ...
    if (moving) {
      stepTowards(); // animation function
    }

    //unused fft peak detection method
    // if ( peakDetect.isDetected ) {
    //     weight = 10;
    //     start()
    // }

    //evoke animation only when definied threshold is exceeded. sensitivity is control factor
  ...
  //Decay weight for splines rapidly
  splineWidth -= 0.5;
}
...
function start() {
  ...
  //assign random values to determine animation direction for each spline vertex
...
//background circle mapped to current level
...
//unused currently for drawing fonts with shape rather than line
function drawMessage() {
  for (let i = 0; i < shapeArray.length; i++) {
    // does not handle 'i' 'j' or '!'
    beginShape();
  ...
  //tl is temporary letters which need to be handled if left over from last word
  if (tl) {
...
//tl is temporary letters which need to be handled if left over from last word
  if (tl) {
    // for left over letters to transition inside others
  ...
  //interpolates position of x and y gradually to target for each vertex according to speed
  for (let i = 0; i < shapeArray.length; i++) {
    for (let j = 0; j < shapeArray[i].length; j++) {
      ...
      if (dist(mover.x, mover.y, target.x, target.y) < 0.1) {
        arrivedCount++;
        //on arrival counts as arrived to allow next transition to begin
...
//set default words to handle null values
function newWord() {
  let n = inp.value();
  if (wordsCount == 0) {
    words[wordsCount] = n;
...
//new array which works for both shape and target arrays structure
function newArr(w) {
  temp = [];
  flag = false;
  fill(255);

  //g library gets path data
...
//simpler drawing for linear fonts
...
//restructures array to be the same as next word
function resizeArray() {
  ...
  if (targetArray.length >= shapeArray.length) {
    ...
    for (let j = 0; j < shapeArray.length; j++) {
      //for length of current word
      if (shapeArray[j].length > targetArray[j].length) {
        //if letter is larger than new letter in this pos
        ...
        for (let i = 0; i < iterations2; i++) {
          //for difference in number of points in the two letters
    ...
    for (let t = 0; t < diff; t++) {
      // handles holding extra letters for moving
      tl = true;
      tempLetters.push([...shapeArray[shapeArray.length - (1 + t)]]); //ensure to clone array so as not to modify
    }
...
function drawSplines() {
  ...
  for (var i = 0; i < vertices.length; i++) {
    ...
    midpointToCenter = createVector(
      midpointToCenter[0] / 4,
      midpointToCenter[1] / 4
    );
...
//used to reset application without refreshing page
function reset() {
  words = [" ", " "];
  first = true;
  currSound.stop();
...
//counts total number of datapoints in multidimensional array
...
//return max and min point in array with highest greatest x or y co-ordinate
function MaxX(arr) {
  let arr2 = [];
